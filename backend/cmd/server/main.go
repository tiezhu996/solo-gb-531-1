...
func run(logger *slog.Logger) error {
	...
	evaluationHandler := handler.NewCoverageEvaluationHandler(service.NewCoverageEvaluationService(
		evaluationRepo, scenarioRepo, nodeRepo, safeguardRepo, conflictRepo, auditRepo, algorithm.NewEvaluator(),
	))
	conflictHandler := handler.NewIndependenceConflictHandler(service.NewIndependenceConflictService(
		conflictRepo, evaluationRepo, auditRepo,
	))
	...
	router.RegisterIndependenceConflictRoutes(api, conflictHandler)
	...
	api.GET("/meta/enums", middleware.RequirePermission(constants.PermissionRead), func(c *gin.Context) {
		util.Success(c, http.StatusOK, gin.H{
			...
			"conflict_resolution_state":  constants.ConflictResolutionStateValues(),
			"conflict_resolution_action": constants.ConflictResolutionActionValues(),
